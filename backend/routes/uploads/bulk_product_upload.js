...
const bulk_product_image_storage = multer.diskStorage({
	...
	destination:function(req, file, cb){
		...
		if (file.fieldname === "just_images_upload") {
			...
			let file_path = path.join(__dirname , '../../assets/bulk_product_images/images')
			cb(null, file_path)	

		} else {

			fs.mkdir( path.join(__dirname , `../../assets/bulk_product_images/${currentDate}_${currentTime}`), { recursive: true }, (err) => {
				if (err) throw err;
			})
			
			let file_path = path.join(__dirname , `../../assets/bulk_product_images/${currentDate}_${currentTime}`)
			cb(null, file_path)	
	...
	bulk_upload_products_with_images(req, res, (err) => {
		if(err){
			...
			try {
				...
				// give path
				let uploaded_excel_sheet = path.join(__dirname , `../../assets/bulk_product_images/${currentDate}_${currentTime}/${req.files['excel_sheet_for_images'][0].filename}`) 
