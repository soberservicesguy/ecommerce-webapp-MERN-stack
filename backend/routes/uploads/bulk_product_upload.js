	...
	bulk_upload_products_with_images( `${currentDate}_${currentTime}`, 'bulk_products' )(req, res, (err) => {
		if(err){
			...
			{(async () => {
				...
				let filepath_in_case_of_disk_storage = get_multer_disk_storage_for_bulk_files(`${currentDate}_${currentTime}`, 'bulk_products')
			...
			// finding the user who is uploading so that it can be passed to sheet_to_class for assignment on posts
				...
				.then((user) => {
					if (user){
						...
						user_id = user._id
						sheet_to_class( excel_filepath, user_id, 'bulk_products',  `${currentDate}_${currentTime}`, ['image_main_filepath'], all_images_db_objects)
